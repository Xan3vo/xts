...
LOG_CHANNEL_ID = 1430037510167199884

CATEGORY_IDS = {
    "getting_info": 1430036848029204511,
    "robux_gamepass": 1430036848029204511,
    "robux_groupfunds": 1430036848029204511,
    "robux_ingame": 1430036848029204511,
...
async def create_ticket_for_user(interaction: discord.Interaction, delivery_type: str, subtype: Optional[str], payment_method: str, amount: int = 0, extra_notes: Optional[str] = None):
    ...
    # Determine category ID and channel name
    category_id = CATEGORY_IDS.get("other")
    subtype_key = None
    if delivery_type.lower() == "robux":
        if subtype == "gamepass":
            category_id = CATEGORY_IDS.get("robux_gamepass")
            subtype_key = "gamepass"
        elif subtype == "groupfunds":
            category_id = CATEGORY_IDS.get("robux_groupfunds")
            subtype_key = "groupfunds"
        elif subtype == "ingame":
            category_id = CATEGORY_IDS.get("robux_ingame")
            subtype_key = "ingame"
        else:
            category_id = CATEGORY_IDS.get("other")
    else:
...
# /edit <payment_method> <text> -> updates payment_info.json
@bot.tree.command(name="edit", description="Edit payment method instructions (admins only)")
@app_commands.describe(payment_method="Payment method key", text="Instruction text")
async def edit_payment(intraction: discord.Interaction, payment_method: str, text: str):
    member = intraction.user
    if not isinstance(member, discord.Member) or not is_admin_member(member):
        await intraction.response.send_message("You do not have permission to run this command.", ephemeral=True)
        return
    key = payment_method.lower()
    data = read_json(PAYMENT_JSON)
    data[key] = text
    write_json(PAYMENT_JSON, data)
    await intraction.response.send_message(f"Updated instructions for **{key}**.", ephemeral=True)
    ...
    if user:
        try:
            dm_message = (
                ...
                "Feel free to vouch at ⁠✓-vouches-3100 💖\n\n"
                "**HOW TO VOUCH:**\n"
...
async def handle_confirmation(user, channel, is_prefix=False, interaction=None):
    ...
    if robux_type == "ingame":
        ...
        embed = discord.Embed(
            ...
            description=(
                "Please send us the following details in regards to your in-game gifting order:\n\n"
                "• **Game / Private Server Link**\n"
                "• **Username of the Recipient**\n"
                "• **What gamepass/item you need gifted**\n"
                "• **Total Robux value of everything**"
            ),
...
# Startup token
# ---------------------------
bot.run("MTQyODUwNjgxNjU1OTU3OTIxNw.GmSV_R.vxhKhiuwQWMfhqO-KvmTjpCwjIdkGxmgRVI0xs")
